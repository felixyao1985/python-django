...
//            校验方式,采用配置信息的思想，而不是把信息写在表单元素上，能比较完美的实现ajax請求。
//----------------------------------------------------------------------------------------------------
// [作者网名] 猫冬	
...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		...
		//如果是精简模式，发生錯誤的时候，第一个錯誤的控件就不获得焦点
		if(settings.tidymode){settings.errorfocus=false};
		
		//灰掉提交按钮的功能
		if(settings.group)
		{
			if(settings.formid!=""){$("#"+settings.formid).submit(function(){if(settings.submitonce){$("#"+settings.formid+" :submit").attr("disabled",true);}})};
		}else{
	...
	setTipState : function(elem,showclass,showmsg)
	{
		...
		if(showclass == 'onFocus' && (showmsg==null || showmsg==""))
		{
		}else{
	...
	//根据单个对象,正確:正確提示,錯誤:錯誤提示
	showMessage : function(returnObj)
	{
	    var id = returnObj.id;
		var elem = $("#"+id).get(0);
		var isvalid = returnObj.isvalid;
		var setting = returnObj.setting;//正確:setting[0],錯誤:对应的setting[i]
		...
		{		
			//验证成功后,如果没有设置成功提示信息,则给出默认提示,否则给出自定义提示;允许為空,值為空的提示
	...
	//验证单个是否验证通过,正確返回settings[0],錯誤返回对应的settings[i]
	oneIsValid : function (id,index)
	{
		var returnObj = new Object();
		returnObj.id = id;
		returnObj.ajax = -1;
		returnObj.errormsg = "";       //自定义錯誤信息
		var elem = $("#"+id).get(0);
	...
	pageIsValid : function (validatorgroup)
	{
		...
		if(isvalid)
		{
			...
			if(!initConfig.group){if(initConfig.submitonce){$(":submit").attr("disabled",true);}}
		}
		else
		{
	...
	ajaxValid : function(returnObj)
	{
		...
		{
			var parm = "clientid="+id+"&"+encodeURIComponent(srcjo.attr('name'))+"="+encodeURIComponent(srcjo.val());
		...
		{	
			...
			beforeSend : function(xhr){
				...
				if(isvalid)
				{
					setting.isvalid = false;		//如果前面ajax請求成功了，再次請求之前先当作錯誤处理
	...
	//函数校验。返回true/false表示校验是否成功;返回字符串表示錯誤信息，校验失败;如果没有返回值表示处理函数，校验成功
		...
		switch(sType)
		{
			...
			case "radio":
				...
				else if (type =="date" || type =="datetime")
				{
					...
					//if(lb_go_on){alert(val);val = new Date(val);setting.min=new Date(setting.min);setting.max=new Date(setting.max);};
				}else{
