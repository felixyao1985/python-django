...
(function($) {
	$.fn.autocomplete = function(params) {
		...
		this.each(function() {
			...
			//Text input		
			var input = $('<input type="text" autocomplete="off" name="'+params.InputName+'" value="'+$(this).text()+'">')
				.attr('placeholder', params.placeholder)
				.addClass(params.className);
			$(this).text('');
			if(params.width>0){
				input.css('width', params.width);
			}else{
				params.width = input.width();
			}
			if(params.height>0){
				input.css('height', params.height);
			}else{
